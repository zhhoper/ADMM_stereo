...
for R = lower_R:upper_R
    ...
    for i = 1:3
        ...
        for out_iter = 1:number_of_out_iter
            ...
            [X_rec(:,:,out_iter),iter_count] = admmAXB(A,B,X,M,known,0.001);
...
imshow(Xrecover/255);
xlabel('recovered image by TNNR-admm');
...
if(to_create_eps_pic)
        ...
        cd result
        saveas(gcf,strcat(image_name,'Rankis',str_R,'PSNRis',num2str(str_error),'ADMM','.eps'),'psc2');
        cd ..
        close all;
end
ret.time = time_cost;
